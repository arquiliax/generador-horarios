...
CATALOGO_MATERIAS = [
    ...
    {"nrc": "58067", "clave": "ISPS 204", "materia": "Terapia Familiar", "secc": "008", "dias": "AJ", "height": "1500-1659", "profesor": "ARCE - MUNOZ MOHAMED", "semestre": "8vo Semestre"},
...
def generar_horario_estricto(lista_materias, profesores_prioritarios, lm_ini, lm_fin, aj_ini, aj_fin, omitir_viernes, max_intentos=5000):
    ...
    for clave, opciones in materias_unicas.items():
        # Identificar las opciones preferidas/prioritarias por el usuario
        ...
        # El conjunto de opciones a evaluar dependerá de si se especificó un profesor prioritario
        opciones_a_evaluar = opciones_con_profesor if usuario_pidio_profesor else opciones
        
        # Filtro del viernes: Evaluamos si se deben omitir
        if omitir_viernes:
            opciones_sin_viernes = [op for op in opciones_a_evaluar if 'V' not in op['dias']]
            
            # Si al quitar los viernes nos quedamos sin opciones para esta materia, significa que 
            # la opción preferida o única requería obligatoriamente un viernes. Se descarta y avisa.
            if len(opciones_sin_viernes) == 0 and len(opciones_a_evaluar) > 0:
                omitidas_por_viernes.append(nombres_materias[clave])
                continue # Omitir por completo la materia
        ...
        if opciones_validas:
            ...
            # Si no hay opciones válidas tras los filtros
    ...
    # Si todo lo disponible se canceló por ser viernes, regresamos una simulación limpia
...
if st.button("🎲 Calcular Horario Óptimo", type="primary"):
    if not lista_materias_trabajo:
        ...
        if calendario is not None:
            ...
            # Mostrar tabla únicamente si el calendario contiene materias resultantes
            if calendario:
