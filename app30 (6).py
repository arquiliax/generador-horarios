...
# Obtener todos los profesores únicos del semestre activo (para el selectbox)
_profesores_semestre = sorted(list({
    m['profesor'].strip()
    for m in CATALOGO_MATERIAS
    if m['semestre'] == semestre_seleccionado and m['profesor'].strip()
}))
_opciones_prof = ["(Seleccionar profesor)"] + _profesores_semestre

profesores_inputs = []
for i in range(1, 9):
    # Solo mostrar el siguiente selectbox si el anterior ya tiene una selección
    if i == 1 or (len(profesores_inputs) == i - 1 and profesores_inputs[-1]):
        prof_sel = st.sidebar.selectbox(
            f"Docente prioritario {i}",
            options=_opciones_prof,
            key=f"prof_sel_{i}",
            index=0
        )
        if prof_sel and prof_sel != "(Seleccionar profesor)":
            ...
            break   # No mostrar más hasta que se llene éste
