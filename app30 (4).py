...
def generar_horario_estricto(lista_materias, optativas_seleccionadas, profesores_prioritarios, lm_lims, aj_lims, v_lims, omitir_viernes, max_intentos=5000):
    ...
    # Si agotamos intentos, identificar qué materias nunca pudieron entrar sin choque
    for clave, opciones in materias_filtradas.items():
        nombre = nombres_materias[clave]
        if nombre not in razones_omision:
    ...
    return None, False, razones_omision, [], omitidas_por_viernes, "No se encontró una combinación válida sin traslapes dentro de las horas de referencia."
