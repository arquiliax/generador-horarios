...
def generar_horario_con_omision(lista_materias, profesores_preferidos, lm_ini, lm_fin, aj_ini, aj_fin, max_intentos=4000):
    materias_unicas = {}
    nombres_materias = {}
    ...
    # Intentar combinación con prioridades
    ...
    # Respaldo aleatorio manteniendo bloqueos
    for _ in range(max_intentos):
        calendario_propuesto = []
        conflicto = False
...
st.set_page_config(page_title="Generador Avanzado de Horarios", layout="wide", page_icon="🗓️")
...
# --- CUERPO PRINCIPAL ---
st.title("🗓️ Generador con Tolerancia a Bloqueos Extremos")
st.subheader("Tercer Semestre")
st.write("Si bloqueas una hora que deje sin opciones a una materia, la aplicación la excluirá automáticamente para salvar el resto de tu horario.")

if st.button("🎲 Calcular Horario Óptimo", type="primary"):
    calendario, prioridad_cumplida, omitidas, mensaje_error = generar_horario_con_omision(
    ...
    if calendario:
        if omitidas:
            ...
            st.success("🎯 ¡Horario completado! Todas las materias del plan de estudios se acomodaron correctamente.")
        ...
        for m in calendario:
            ...
            # CORREGIDO: Ahora todas las asignaciones apuntan correctamente a la variable local df_horario
