...
# =========================================================================
# 🧬 CATÁLOGO DE MATERIAS OPTATIVAS POR ÁREA ACADÉMICA
# =========================================================================
OPTATIVAS_POR_AREA = {
    "Educativa": [
...
def generar_horario_estricto(lista_materias, optativas_seleccionadas, profesores_prioritarios, lm_ini, lm_fin, aj_ini, aj_fin, omitir_viernes, max_intentos=5000):
    # Consolidar materias base únicas por clave
    materias_unicas = {}
    nombres_materias = {}
    ...
    # Evaluar filtros sobre materias Base
    ...
    # Filtrar las optativas que el usuario marcó manualmente basándonos en los bloqueos/viernes
    optativas_filtradas = {}
    for clave_opt, opciones_opt in optativas_seleccionadas.items():
        ...
        if omitir_viernes:
            opciones_a_evaluar = [op for op in opciones_a_evaluar if 'V' not in op['dias']]

        opciones_validas = [op for op in opciones_a_evaluar if not choca_con_bloqueo_por_dia(op, lm_ini, lm_fin, aj_ini, aj_fin)]
        if opciones_validas:
            optativas_filtradas[clave_opt] = opciones_validas

    # Si el usuario quiere llevar optativas, pero los bloqueos eliminaron sus opciones válidas:
    if optativas_seleccionadas and not optativas_filtradas:
         return None, False, materias_omitidas, omitidas_prof_unico, omitidas_por_viernes, "Las restricciones de bloqueo eliminaron las opciones de las optativas seleccionadas."
    ...
    # Ciclo de fuerza bruta combinatoria
    for _ in range(max_intentos):
        calendario_propuesto, conflicto = [], False
        
        # 1. Añadir una sección de cada materia base mapeada
        ...
        # 2. Añadir una sección de cada materia optativa mapeada
    ...
    return None, False, materias_omitidas, omitidas_prof_unico, omitidas_por_viernes, "No se encontró una combinación válida sin traslapes horarios entre tus materias base y optativas."
...
# =========================================================================
# ✨ NUEVA SECCIÓN DE SELECCIÓN DE OPTATIVAS (SIDEBAR)
# =========================================================================
st.sidebar.markdown("---")
...
with optativas_expander:
    st.markdown("<small>Selecciona las asignaturas optativas que deseas incorporar a tu carga:</small>", unsafe_allow_html=True)
    
    for area, materias in OPTATIVAS_POR_AREA.items():
        st.markdown(f"**Área {area}**")
        
        # Agrupar las secciones por el nombre único de la materia para no duplicar la selección de la asignatura
        ...
        for nom_materia, opciones in materias_unicas_nombre.items():
            # Generamos un checkbox para cada materia optativa única
            marcado = st.checkbox(nom_materia, key=f"opt_{opciones[0]['clave']}")
            if marcado:
...
if st.button("🎲 Calcular Horario Óptimo", type="primary"):
    ...
    # 🚨 VALIDACIÓN DE LÍMITE DE MATERIAS (Base Esperadas + Optativas Elegidas)
    ...
    if total_materias_solicitadas > 8:
    ...
    elif not lista_materias_trabajo:
        ...
        if calendario is not None:
            ...
            if len(profesores_inputs) > 0 and len(calendario) > 0:
                ...
                st.success(f"Target obtenido con éxito. Carga total final armada: {len(calendario)} materias.")
