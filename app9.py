...
CATALOGO_MATERIAS = [
    ...
    {"nrc": "57895", "clave": "ISPS 210", "materia": "Estrategias de Inte en Psi Cli", "secc": "008", "dias": "AJ", "height": "1500-1629", "profesor": "GALINDO-MOTO MANUEL ALEJANDRO", "semestre": "10mo Semestre"},
...
def generar_horario_estricto(lista_materias, profesores_prioritarios, lm_ini, lm_fin, aj_ini, aj_fin, omitir_viernes, max_intentos=5000):
    ...
    for clave, opciones in materias_unicas.items():
        # Filtrar si el usuario activó la exclusión estricta de viernes
        if omitir_viernes:
            opciones = [op for op in opciones if 'V' not in op['dias']]
    ...
    if not materias_filtradas:
        return None, False, materias_omitidas, omitidas_prof_unico, "Las restricciones de bloqueo o la exclusión de viernes eliminaron todas las opciones válidas."
...
# Aquí añadimos el checkbox que solicitaste
...
if st.button("🎲 Calcular Horario Óptimo", type="primary"):
    if not lista_materias_trabajo:
        ...
        # Enviamos el parámetro 'omitir_viernes' al motor de generación
        ...
        if calendario:
            if om:
                st.warning(f"⚠️ **Atención:** Para cumplir tus restricciones de tiempo/días, se omitieron: {', '.join(om)}.")
            if om_prof:
                st.error(f"👤 **Materia Omitida:** La asignatura **{', '.join(om_prof)}** se omitió porque el profesor prioritario choca con tus bloqueos o tiene clases los viernes.")
            ...
            if len(profesores_inputs) > 0:
                if prioridad_cumplida:
                    ...
                    st.warning("⚠️ **Filtro No Aplicado Completamente:** Ciertos profesores prioritarios no se incluyeron por cruce con horas bloqueadas o viernes.")
            else:
