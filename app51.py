...
def generar_horario_prioritario(lista_materias, profesores_prioritarios, lm_ini, lm_fin, aj_ini, aj_fin, max_intentos=4000):
    ...
    if not materias_filtradas:
        return None, False, materias_omitidas, "El bloqueo de horas eliminó absolutamente todas las materias del semestre."

    # Intentar combinación con PRIORIDAD ESTRICTA (Obligatoria si se especifica)
    for _ in range(max_intentos):
        calendario_propuesto = []
        materias_restantes = list(materias_filtradas.keys())
        conflicto = False

        # PASO 1: Colocar estrictamente a los profesores asignados prioritarios
        for clave in list(materias_restantes):
            opciones = materias_filtradas[clave]
            # Filtrar si esta materia tiene una sección con los profesores que escribió el usuario
        ...
        # PASO 2: Rellenar de forma completamente aleatoria el resto de materias libres
        ...
        if not conflicto and len(calendario_propuesto) == len(materias_filtradas):
            return calendario_propuesto, True, materias_omitidas, ""

    # PLAN DE RESPALDO: Si los profesores prioritarios chocan matemáticamente entre sí o con el bloqueo, 
    # generamos el mejor horario que proteja tus horas bloqueadas aunque deba variar algún docente.
    for _ in range(max_intentos):
        calendario_propuesto = []
        conflicto = False
        ...
        if not conflicto and len(calendario_propuesto) == len(materias_filtradas):
            return calendario_propuesto, False, materias_omitidas, ""

    return None, False, materias_omitidas, "No se encontró una combinación válida libre de traslapes con las materias restantes."
...
# SECCIÓN MODIFICADA: PROFESORES PRIORITARIOS DIRECTOS
st.sidebar.markdown("---")
st.sidebar.subheader("👤 Profesores Prioritarios")
st.sidebar.write("Las materias de los docentes listados abajo se volverán prioritarias en el algoritmo:")
profesores_inputs = []
for i in range(1, 9):
    pref_name = st.sidebar.text_input(f"Docente Prioritario {i}", key=f"prof_{i}", placeholder="Ej. Durán o Yadira")
    if pref_name.strip():
...
st.write("Escribe el nombre de tus profesores para forzar su inclusión prioritaria. Las materias sin profesor asignado se decidirán al azar.")

if st.button("🎲 Calcular Horario Óptimo", type="primary"):
    calendario, prioridad_cumplida, omitidas, mensaje_error = generar_horario_prioritario(
    ...
    if calendario:
        if omitidas:
            ...
            st.warning(f"⚠️ **Atención:** Para poder cumplir con tu restricción de horas bloqueadas, se tuvieron que **omitir de forma obligatoria** las siguientes materias: {', '.join(omitidas)}.")
        else:
            st.success("🎯 ¡Horario estructurado correctamente!")

        if len(profesores_inputs) > 0:
            if prioritario_cumplida := prioridad_cumplida:
                st.balloons()
                st.info("💎 **Filtro Aplicado:** Se fijaron exitosamente tus profesores prioritarios y el resto de materias se completaron de forma aleatoria.")
            else:
                st.warning("⚠️ **Nota:** Se respetaron tus horas prohibidas, pero se tuvo que modificar algún profesor prioritario debido a un choque insalvable de horarios en sus secciones.")
